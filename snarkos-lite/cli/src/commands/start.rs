...
pub struct Start {
    ...
    #[clap(default_value_os_t = std::env::temp_dir().join("snarkos-lite.log"), long = "logfile")]
    pub logfile: PathBuf,
...
impl Start {
    ...
    fn parse_genesis<N: Network>(&self) -> Result<Block<N>> {
        ...
        let development_private_keys =
            (0..DEVELOPMENT_MODE_NUM_GENESIS_COMMITTEE_MEMBERS).map(|_| PrivateKey::<N>::new(&mut rng)).collect::<Result<Vec<_>>>()?;
        ...
        let public_balance_per_validator = remaining_balance.saturating_div(DEVELOPMENT_MODE_NUM_GENESIS_COMMITTEE_MEMBERS as u64);
        ...
        let leftover =
            remaining_balance.saturating_sub(public_balance_per_validator * DEVELOPMENT_MODE_NUM_GENESIS_COMMITTEE_MEMBERS as u64);
        if leftover > 0 {
        ...
        compute_genesis(development_private_keys[0], committee, public_balances, bonded_balances, &mut rng)

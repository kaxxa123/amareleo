...
// Bin Name:   snarkos -> snarkos-lite
// Repo Name:  snarkOS -> snarkOS-lite
...
impl Updater {
    const SNARKOSLITE_BIN_NAME: &'static str = "snarkos-lite";
    const SNARKOSLITE_REPO_NAME: &'static str = "snarkOS-lite";
    const SNARKOSLITE_REPO_OWNER: &'static str = "kaxxa123";

    /// Show all available releases for `snarkos-lite`.
    ...
    /// Update `snarkOS-lite` to the specified release.
    ...
    /// Check if there is an available update for `snarkos-lite` and return the newest release.
    ...
    pub fn print_cli() -> String {
        if let Ok(latest_version) = Self::update_available() {
            ...
            output += &" `snarkos-lite update` ".bold().white();
